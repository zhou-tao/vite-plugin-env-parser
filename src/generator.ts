...
import type { EnvParserOptions, ViteEnv, ParsedValue } from './types'

export function generateDTS({ dts, injectViteDTS }: EnvParserOptions, root = process.cwd(), env: ViteEnv = {}) {
  if (!dts) return
  dts = dts === true ? 'src/env.d.ts' : dts
  const path = resolve(root, dts as string)
  const doWriteFileApi = writeFileSync
  doWriteFileApi(path, createDTSContent(env, injectViteDTS), 'utf-8')
}
...
function createDTSContent(env: ViteEnv = {}, injectViteDTS?: boolean): string {
